...
(function($) {
	...
	Calendar.prototype._calculate_hour_minutes = function(data) {
		...
		$.each(data.events, function(k, e) {
			...
			//Implemented in day.html and week.html.  Edited to displays the start and end date&time for events.
			if(e.start < e.end_hour) {
				warn(1);
				if(s.getHours() < 12	) {
					e.start_hour = $self.locale['ms' + s.getMonth()] + ' ' + s.getDate() + ' ' + e.start_hour+ ' ' + 'AM';
				}
				else {
					e.start_hour = $self.locale['ms' + s.getMonth()] + ' ' + s.getDate() + ' ' + e.start_hour + ' ' + 'PM';
					}			
				}
			...
			if(e.end > end.getTime()) {
				warn(1);
				if(f.getHours() < 12	) {
					e.end_hour = $self.locale['ms' + f.getMonth()] + ' ' + f.getDate() + ' ' + e.end_hour + ' ' + 'AM';
				}
				else {
					e.end_hour = $self.locale['ms' + f.getMonth()] + ' ' + f.getDate() + ' ' + e.end_hour + ' ' + 'PM';
				}
			}
	...
	Calendar.prototype._hour = function(hour, part) { //MODIFIED TO DISPLAY "AM" AND "PM" 
		var time_start = this.options.time_start.split(":");
		var time_split = parseInt(this.options.time_split);
		...
		if (hour < 6) {
			return h.formatNum(2) + ":" + m.formatNum(2) + "AM";
		}
		else {
			return h.formatNum(2) + ":" + m.formatNum(2) + "PM";
		}
	};
